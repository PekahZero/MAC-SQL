...
def init_bird_message(
    idx: int, item: dict, db_path: str = None, use_gold_schema: bool = False
) -> dict:
    """
    Construct message for text-to-SQL task
    :param idx: start from 0
    ...
    :return: initial message object of group chat
    """
    db_id, query, evidence, gt, difficulty = (
        item["db_id"],
        item["question"],
        item["evidence"],
        item.get("SQL", ""),
...
def init_science_message(
    ...
    db_id, query, evidence, gt, difficulty = (
        ...
        item.get("SQL", ""),
        item.get(
            "difficulty", item["db_id"]
        ),  # science 没有苦难程度信息, 使用数据库的信息代替
    )

    gold_schema_path = "./data/science/dev_gold_schema.json"
    gold_schema = {}
    all_gold_schema_dict = {}
    key = f"{db_id.strip()}\t{query.strip()}"
    if use_gold_schema:
...
def run_batch(
    ...
    # print("Start to run batch...")
    chat_manager.ping_network()
    # load dataset
    ...
    unfinished_ids = [
        n for n in range(len(batch)) if n not in finished_ids and n >= start_pos
    ]
    ...
    with open(output_file, "a+", encoding="utf-8") as fp:
        ...
        for cur_idx, item in tqdm(enumerate(batch), total=total_num):
            ...
            if dataset_name == "spider":
                user_message = init_spider_message(
                    idx, item
            ...
            # SCIENCE
                ...
                user_message = init_science_message(
                    idx, item, db_path=db_path, use_gold_schema=use_gold_schema
                )  # imitate user send a question to system
            try:
                ...
                # for debug
                traceback.print_exc()
                print(f"Exception: {e}, sleep 10 seconds.", flush=True)
                time.sleep(10)
    ...
    # SCIENCE
        ...
        evaluation_file_path_SQL = f"{out_dir}/pred_{dataset_mode}.sql"
        spider_sql_lst = []
        ...
        save_file(evaluation_file_path_SQL, spider_sql_lst)
        ...
        with open(evaluation_file_path, "w", encoding="utf8") as fout:
            ...
            for o in output_json_list:
                ...
                pred_sql = replace_multiple_spaces(pred_sql)
                sql_and_db_str = (
                    pred_sql + "\t" + "-----science-----" + "\t" + o["db_id"]
                )
                obj = [o["query"], sql_and_db_str]
                eval_tuple_lst.append(obj)
            json.dump(eval_tuple_lst, fp=fout, ensure_ascii=False, indent=2)

        print(f"SCIENCE format file dump into {evaluation_file_path}")
    # SPIDER
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--db_path", type=str, required=True, help="path to databases in dataset"
    )
    parser.add_argument(
        "--tables_json_path", type=str, default=None, help="path to tables.json"
    )
    parser.add_argument(
        "--output_file", type=str, required=True, help="path to predicted output"
    )
    parser.add_argument(
        "--log_file", type=str, default="", help="path to log file if needed"
    )
    parser.add_argument(
        "--start_pos", type=int, default=0, help="start position of a batch"
    )
